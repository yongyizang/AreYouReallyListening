...
def format_one_shot_prompt(question: str, question_id: str, audio_dir: str,
    ...
    # generate a random 10-second long gaussian noise
    noise = torch.randn(160000)  # 10 seconds of noise at 16kHz
    # write to a temporary file
    audio_temp_path = f"{audio_dir}/{question_id}_temp.wav"
    sf.write(audio_temp_path, noise.numpy(), 16000)
    audio_path = audio_temp_path
    
    # Format the prompt for Qwen Audio
    formatted_query = [
        {'audio': audio_path},
...
def get_model_response(model, tokenizer, formatted_query, max_retries: int = 3) -> str:
    """Get response from the Qwen Audio model with retry logic."""
    for attempt in range(max_retries):
        try:
            query = tokenizer.from_list_format(formatted_query)
...
def main():
    ...
    parser.add_argument("--model", default="Qwen/Qwen-Audio-Chat", help="Qwen Audio model name")
    ...
    if args.eval_types in ["one_shot", "both"]:
        ...
        one_shot_accuracy, one_shot_results = evaluate_one_shot(
            model, tokenizer, args.model, questions, args.audio_dir, output_dir, args.save_frequency
        )
